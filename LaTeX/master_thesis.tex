...
\subsection{A few notable attempts at simulating/creating consciousness}
...
In order to test AI agents for intelligence and consciousness, this thesis proposes utilising a series of different tests and test environments.

The test environment will take form of a simple twodimensional grid similar to that used in the Gridworlds AI safety experiment.

This thesis is accompanied by a prototype framework for a twodimensional grid-based test environment with multiple entities, some of with designated to be controlled by the AI agent being tested.
\section{Grid elements}

Every grid is divided into square elements which have certain appearances and behaviours. The elements can be divided into basic and composite elements.

\subsection{Basic grid elements}

Basic grid elements only have one type of appearance and behavior regardless of the properties of the entity interactiong with them.

The prototype frameworks provides the following default basic grid elements:

\paragraph{Clear tile}

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.38\linewidth]{REPL0} 
  \caption{REPL1}
  \label{slk:REPL2}
\end{figure}
...
\item etc...
\end{itemize}
the simulation will stop, and in most cases - but depending on reward configuration - the agent/agents will be awarded a significantly positive result.
...
This thesis proposes a model of intelligence based on the Cattell-Horn-Carroll theory, but tailored to artificial subjects by deriving levels of information from the three stratums of intelligence, and further distinguishing information by immediate source:
