...
Changes the point of view from which the environment is displayed, with all entities as well as an omniscient viewpont (default) as options.
...
Allow hiding and re-showing the grid and the entities.
...
The developed prototype allows simple implementation of more complex grid environment, such as one that requires an agent to solve a Raven Progressive Matrix test in order to navigate the environment safely by hiding a pattern of hazards and requiring the agent to deduce it from available information in RPM form.
\paragraph{More complex environment development}
A simple grid environment system, while capable of supporting simplified versions of most conceivable AI tests, cannot be considered state-of-the-art due to existence of more complex test environments, whether purpose-built for AI testing or repurposed such as the open-world games used to test SIMA.
\section{Discussion}
\subsection{Ethical concerns}

As research progresses, it is possible artificial intelligence reaches the point where its instances could qualify for ethical protection levels granted to certain animal test subjects, which may make some research methods unethical and require additional caution in early stages of AI agent development to reduce unnecessary artificial suffering.
...
\end{quote}
In order to develop a method to effectively test for artificial general intelligence or consciousness, it may be necessary to develop increasingly complex artificial agents. This process, similar to that of gain-of-function research in virology, could pose relatively minor but significant safety risks if safety is ignored or mismanaged.
...
As regulation of software development may be significanly more difficult than that of nuclear weapons the only way to alleviate this dilemma is to work on making safe AI easier to implement, but also invest a significant amount of resources in safe AGI research in order to develop the first AGI without compromising on safety.
