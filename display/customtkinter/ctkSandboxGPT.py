...
class GridFrame(ctkDiB.iTkFrameDef):
    """
    Frame for displaying a grid.
    """

    def __init__(self, master, return_lambda: callable, screen_size: tuple[int, int],
                 images: tuple[list[ctkDiB.GridElementDisplay], list[ctkDiB.GridElementDisplay]] = None):
        ...
        self.canvas = None
        if images is None:
            images = ctkDiB.get_grid_tile_images()
        self.images = images
    ...
    def create_widgets(self):
        ...
        self.tile_images: list[ctkDiB.GridElementDisplay] = self.images[0]
        self.agent_images: list[ctkDiB.GridElementDisplay] = self.images[1]
...
class GridButtonFrame(ctkDiB.iTkFrameDef):

    def create_widgets(self):
        size = (self.screen_size[0], 50)
        X = ctkDiB.InputFrame(self, lambda E: self.return_lambda("run." + str(E)), size, str.isdigit, 1).ret_pack()
        self.widgets["iterate"] = X
        console = ctkDiB.GridConsole(self, "Console", self.return_lambda, (self.screen_size[0],) * 2)
        console.pack()
        ...
        for name, values in X:
            X = ctkDiB.SelectFrame(self, self.return_lambda, (0, 0), name, values).ret_pack()
            self.widgets[name] = X
...
class GridDisplayFrame(ctkDiB.iTkFrame):
    def __init__(self, master: ctkDiB.SwapFrame, name="GridDisplayFrame", screen_size=(800, 700)):
        self.agent_looks = {}
        self.view_elements_mode = {"Grid", "Agents"}
        self.agent_locations = dict()
        self.winStatus = (None, 0)
        screen_size = (800, 700)
        self.env: [ctkDiB.GridEnvironment, None] = None
        self.obs_agent = None
    ...
    def create_widgets(self):
        ...
        self.grid_display = GridFrame(self, self.return_lambda, gridsize, ctkDiB.get_grid_tile_images())
    ...
    def make_iter_text(self):
        ...
        self.env: ctkDiB.GridEnvironment
    ...
    def set_env(self, env: ctkDiB.GridEnvironment = None, init=False):
        self.env = env
    ...
    def check_entity_locations(self, seen: dict):
        env: ctkDiB.GridEnvironment
        env = self.env
        entities: list[ctkDiB.GridEntity]
    ...
    def update_env(self, animation_steps:int=2):
        env: ctkDiB.GridEnvironment = self.env
        data: dict
        locations={}
        if env is None:
...
def main():
    root = tk.Tk()
    disp = GridFrame(root, print, (600, 600), ctkDiB.get_grid_tile_images())
